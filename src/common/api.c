...
static SDL_Rect asset_rects[] = {
    ...
    [ASSET_SWAP]             = (SDL_Rect){SCALE4(95,51,10,10)},
    [ASSET_SWAP_GRAY]        = (SDL_Rect){SCALE4(105,51,10,10)},
    [ASSET_SWAP_DARK_GRAY]   = (SDL_Rect){SCALE4(114,51,10,10)},
};
 ...
 * Sets up SDL video, input, colors, assets, and fonts.
 */
SDL_Surface* GFX_init(int mode) {
    ...
    // Map standard colors to the screen format
...
/**
 * Shuts down the graphics subsystem and releases resources.
 */
void GFX_quit(void) {
 ...
 * Handles VSync logic if strict vsync is enabled.
 */
...
/**
 * Manual synchronization to maintain 60fps if GFX_flip wasn't called.
 */
void GFX_sync(void) {
...
// -------------------------------------------------------------------------
// Anti-Aliased Scaling Logic (Software)
// -------------------------------------------------------------------------
...
/**
 * Performs anti-aliased scaling of an image.
 * Uses a weighted average approach to blend pixels when downscaling or upscaling.
 */
...
int GFX_blitHardwareGroup(SDL_Surface* dst, int show_setting) {
    ...
    if (show_setting && !GetHDMI()) {
        ...
        //printf("GFX_blitHardwareGroup: show_setting(%i) val: %i (%i<->%i)\n", show_setting, setting_value, setting_min, setting_max);
...
// -------------------------------------------------------------------------
// SND - Audio Subsystem (PicoArch based)
// -------------------------------------------------------------------------
...
/**
 * SDL Audio callback. Fills the stream with samples from the ring buffer.
 */
...
void SND_init(double sample_rate, double frame_rate) { // plat_sound_init
    ...
    LOG_info("SND_init: calling SDL_InitSubSystem...\n");
    SDL_InitSubSystem(SDL_INIT_AUDIO);
    LOG_info("SND_init: SDL_InitSubSystem OK\n");
    ...
    // clean sdlspec
    memset(&spec_in, 0, sizeof(SDL_AudioSpec));
    ...
    LOG_info("SND_init: Calling SDL_OpenAudio with freq %d...\n", spec_in.freq);
    if (SDL_OpenAudio(&spec_in, &spec_out) < 0) {
        LOG_info("SDL_OpenAudio error: %s\n", SDL_GetError());
    } else {
        LOG_info("SND_init: SDL_OpenAudio OK\n");
    }
    ...
    LOG_info("SND_init: sample rate: %i (req) %i (rec)\n", snd.sample_rate_in, snd.sample_rate_out);
    snd.initialized = 1;
}
...
/**
 * Polls for input events (SDL keys, joysticks) and updates pad state.
 * Implements a simple repeat logic for buttons held down.
 */
void PAD_poll(void) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type==SDL_KEYDOWN || event.type==SDL_KEYUP) {
            ...
            pressed = event.type==SDL_KEYDOWN;
             //LOG_info("key event: %i (%i) [%i]\n", code,pressed, CODE_POWER);
...
// -------------------------------------------------------------------------
// PWR - Power Management (Sleep, Battery, Settings overlay)
// -------------------------------------------------------------------------
...
/**
 * Updates power state, handles auto-sleep timers, and toggles settings overlays.
 * Checks for power button presses and inactivity.
 */
...
static void PWR_waitForWake(void) {
    ...
    while (!wake) {
        ...
        SDL_Delay(200);
        if (pwr.can_poweroff && SDL_GetTicks()-sleep_ticks>=300000) { // five minutes
