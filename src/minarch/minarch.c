...
void __log_finite(void){}

// default frontend options
static int screen_scaling = SCALE_ASPECT;
static int screen_sharpness = SHARPNESS_CRISP;
...
static struct Core {
    ...
    const bool core_overlay; // has a overlay.png on their PKG
...
///////////////////////////////////////
// Definimos un tipo para el hash (32 bits es suficiente para esto)
typedef unsigned long sram_hash_t;

// Variable estática para guardar el último hash conocido del disco
static sram_hash_t last_sram_hash = 0;
static int sram_hash_initialized = 0; // Para saber si ya leímos alguna vez

// --- ALGORITMO DE HASH FNV-1a (Implementación pura en C) ---
// No requiere librerías externas, tablas ni mucha memoria.
static sram_hash_t calculate_sram_hash(const void* data, size_t size) {
    const unsigned char* p = (const unsigned char*)data;
    sram_hash_t hash = 2166136261UL; // Offset basis FNV
...
static void SRAM_read(void) {
    ...
    if (!sram || !fread(sram, 1, sram_size, sram_file)) {
        ...
        // Si falla la lectura, reseteamos el hash para forzar escritura futura
        sram_hash_initialized = 0;
    } else {
        // ÉXITO: Calculamos el hash de lo que acabamos de cargar
        // Así sabemos qué hay exactamente en el disco.
        last_sram_hash = calculate_sram_hash(sram, sram_size);
        sram_hash_initialized = 1;
    }
...
static void SRAM_write(void) {
    ...
    // 1. Calculamos el hash del estado ACTUAL de la memoria
    sram_hash_t current_hash = calculate_sram_hash(sram, sram_size);

    // 2. Comparamos con el hash de la última vez que tocamos el disco
    if (sram_hash_initialized && current_hash == last_sram_hash) {
        LOG_info("IGN! sav path (write)\n");
        // Son iguales matemáticamente, no ha cambiado nada. Salimos.
        return;
    }

    // Si llegamos aquí, es porque los hashes son distintos. Toca escribir.
    char filename[MAX_PATH];
    SRAM_getPath(filename);
    ...
    if (sram_size != fwrite(sram, 1, sram_size, sram_file)) {
        ...
        // ÉXITO: Actualizamos el hash de referencia
        last_sram_hash = current_hash;
        sram_hash_initialized = 1;
    }
...
#define LOCAL_BUTTON_COUNT 23 // depends on device
#define RETRO_BUTTON_COUNT 23 // allow L3/R3 to be remapped by user if desired, eg. Virtual Boy uses extra buttons for right d-pad
typedef struct {
    const char* name;
    int key;
} KeyMapping;

static const KeyMapping key_label_mapping[] = {
{"RETURN", RETROK_RETURN},
    ...
    // Numpad
    {"KP0", RETROK_KP0}, {"KP1", RETROK_KP1}, {"KP2", RETROK_KP2}, {"KP3", RETROK_KP3},
    {"KP4", RETROK_KP4}, {"KP5", RETROK_KP5}, {"KP6", RETROK_KP6}, {"KP7", RETROK_KP7},
    {"KP8", RETROK_KP8}, {"KP9", RETROK_KP9},
    ...
    {"F13", RETROK_F13}, {"F14", RETROK_F14}, {"F15", RETROK_F15},
    ...
    // Modificadores y Teclas Especiales
    {"NUMLOCK", RETROK_NUMLOCK},
    {"CAPSLOCK", RETROK_CAPSLOCK},
    {"SCROLLOCK", RETROK_SCROLLOCK},
    {"RSHIFT", RETROK_RSHIFT},
    {"LSHIFT", RETROK_LSHIFT},
    {"SHIFT", RETROK_LSHIFT}, // Alias para shift genérico
    {"RCTRL", RETROK_RCTRL},
    {"LCTRL", RETROK_LCTRL},
    {"CTRL", RETROK_LCTRL},   // Alias para ctrl genérico
    {"RALT", RETROK_RALT},
    {"LALT", RETROK_LALT},
    {"ALT", RETROK_LALT},     // Alias para alt genérico

    {"PAUSE", RETROK_PAUSE},
    {"ENTER", RETROK_RETURN}, // Alias de return
    ...
    {"CARET", RETROK_CARET},           // ^
    {"UNDERSCORE", RETROK_UNDERSCORE}, // _
    ...
    {"RMETA", RETROK_RMETA},
    {"LMETA", RETROK_LMETA},
    {"LSUPER", RETROK_LSUPER}, // Windows Key izquierda
    {"RSUPER", RETROK_RSUPER}, // Windows Key derecha
    {"MODE", RETROK_MODE},
...
// Mapea directamente un botón físico de la consola a una tecla de teclado
static KeyBind physical_to_retro_key[LOCAL_BUTTON_COUNT];
...
static void Config_init(void) {
    ...
    while ((tmp = strstr(tmp, "bind "))) {
        // avoid more binds than the struct
        if (i >= RETRO_BUTTON_COUNT) {
            LOG_info("Warning: Max bindings reached. Ignoring remaining binds.\n");
            break;
        }
        ...
        LOG_info("\t<%i> bind %s (%s) %i:%i\n", i, button_name, button_id, local_id, retro_id);
...
static void Config_quit(void) {
    ...
    for (int i=0; core_button_mapping[i].name; i++) {
        LOG_info("\tClean <%i> bind %s\n", i, core_button_mapping[i].name);
...
static void Config_readControlsString(char* cfg) {
    ...
    // --- BUCLE ÚNICO PARA MANDOS Y TECLADO ---
    for (int i=0; config.controls[i].name; i++) {
        ButtonMapping* mapping = &config.controls[i];
        
        // 1. LEER EL BOTÓN DE MANDO NORMAL (ej: bind Start = b)
        sprintf(key, "bind %s", mapping->name);
        sprintf(value, "NONE");
        
        if (Config_getValue(cfg, key, value, NULL)) {
            if ((tmp = strrchr(value, ':'))) *tmp = '\0'; // artefacto de default.cfg
        ...
        // 2. LEER LA TECLA DEL TECLADO ASIGNADA (ej: bind_key Start = return)
        sprintf(key, "bind_key %s", mapping->name);
        sprintf(value, "NONE");
        
        //mapping->retro_key = 0; // Limpiamos por si cargamos un nuevo cfgç
        
        if (Config_getValue(cfg, key, value, NULL)) {
            if (!strcmp(value, "NONE")) {
                mapping->retro_key = 0; // Solo lo borramos si el cfg dice "NONE" explícitamente
            } else {
                for (int j=0; key_label_mapping[j].name; j++) {
                    if (!strcasecmp(key_label_mapping[j].name, value)) {
                        mapping->retro_key = key_label_mapping[j].key;
                        LOG_info("KEY MAPPED: %s -> %s (Key ID: %i)\n", mapping->name, value, mapping->retro_key);
                        break;
                    }
                }
            }
...
static void Config_write(int override) {
    ...
    // --- BUCLE DE CONTROLES MANDO Y TECLADO ---
    for (int i=0; config.controls[i].name; i++) {
        ButtonMapping* mapping = &config.controls[i];
        ...
        // Si este botón tiene una tecla asignada, guardamos SOLO el bind_key
        if (mapping->retro_key > 0 && mapping->retro_key < RETROK_LAST) {
            for (int k = 0; key_label_mapping[k].name; k++) {
                if (key_label_mapping[k].key == mapping->retro_key) {
                    fprintf(file, "bind_key %s = %s\n", mapping->name, key_label_mapping[k].name);
                    break;
                }
            }
        } 
        // Si NO tiene tecla asignada, guardamos el comportamiento normal del mando
        else {
...
static uint32_t keyboard_state[RETROK_LAST];
static uint32_t current_input_frame = 0;
...
static int16_t input_state_callback(unsigned port, unsigned device, unsigned index, unsigned id) {
    ...
    if (port == 0 && device == RETRO_DEVICE_JOYPAD && index == 0) {
        ...
        return (keyboard_state[id] == current_input_frame) ? 1 : 0;
    }
    return 0;
}
///////////////////////////////
...
static void Input_init(const struct retro_input_descriptor *vars) {
    ...
    for (int i=0; config.controls[i].name; i++) {
        ...
        if (mapping->retro < 0) {
            ...
            LOG_info("KEYBOARD BIND: %s -> <%s> (Key: %i)\n", mapping->name, (mapping->local==BTN_ID_NONE ? "NONE" : device_button_names[mapping->local]), mapping->retro_key);
            continue; // ¡Saltamos al siguiente para evitar el present[-1]!
        }
...
static void selectScaler(int src_w, int src_h, int src_p) {
    ...
    if (scaling==SCALE_NATIVE || scaling==SCALE_CROPPED) {
        ...
        // TODO: eg. src.size + src.clip + dst.size + dst.clip
            ...
            sprintf(scaler_name, "cropped_%i", scale);

            int scaled_w = src_w * scale;
            int scaled_h = src_h * scale;

            dst_w = scaled_w > FIXED_WIDTH? FIXED_WIDTH: scaled_w;
            dst_h = scaled_h > FIXED_HEIGHT? FIXED_HEIGHT:  scaled_h;
            dst_p = dst_w * 2;

            int ox = (DEVICE_WIDTH  - scaled_w) / 2; // may be negative
            int oy = (DEVICE_HEIGHT - scaled_h) / 2; // may be negative

            LOG_debug("cropped: W(%i) - SCW(%i) / 2 = %i\n", DEVICE_WIDTH, scaled_w, ox);
            if (ox<0) {
                src_x = -ox / scale;
                src_w -= src_x * 2;
            }
            ...
            sprintf(scaler_name, "integer_%i", scale);
            int scaled_w = src_w * scale;
            int scaled_h = src_h * scale;
        ...
        // these both will use a generic nn scaler
            sprintf(scaler_name, "fullscreen");
            dst_w = DEVICE_WIDTH;
            dst_h = DEVICE_HEIGHT;
            dst_p = DEVICE_PITCH;
        ...
        scale = scale_f==1.0 ? 1 : -1;
            ...
            sprintf(scaler_name, "aspect");
            scale = 0;

            double device_aspect = ((double)DEVICE_WIDTH) / DEVICE_HEIGHT;
            int scaled_w, scaled_h;

            if (core.aspect_ratio >= device_aspect) {
                scaled_w = DEVICE_WIDTH;
                scaled_h = (int)(DEVICE_WIDTH / core.aspect_ratio);
            } else {
                scaled_h = DEVICE_HEIGHT;
                scaled_w = (int)(DEVICE_HEIGHT * core.aspect_ratio);
            }
            ...
            LOG_debug("scaled: SCW(%i)/SCH(%i) dst_x (%i - %i)/2 = %i | dst_y (%i - %i)/2 = %i\n", scaled_w, scaled_h, DEVICE_WIDTH, scaled_w, dst_x, DEVICE_HEIGHT, scaled_h, dst_y);
    }
    ...
    if (screen->w!=dst_w || screen->h!=dst_h || screen->pitch!=dst_p) {
...
void Core_open(const char* core_path, const char* tag_name) {
   ...
   LOG_info("Retro core.config_dir %s\n", core.config_dir);
   LOG_info("Retro core.states_dir %s\n", core.states_dir);
   LOG_info("Retro core.saves_dir %s\n", core.saves_dir);
   LOG_info("Retro core.bios_dir %s\n", core.bios_dir);
...
// --- INICIO DE IMPLEMENTACIÓN DEL MENÚ DE TECLADO ---

static char** key_labels = NULL;
static int key_labels_count = 0;

static void init_key_labels(void) {
    if (key_labels) return;
    
    // Contamos las teclas del diccionario
    while (key_label_mapping[key_labels_count].name) {
        key_labels_count++;
    }
    
    // Asignamos memoria (0="NONE", 1=primera tecla...)
    key_labels = calloc(key_labels_count + 2, sizeof(char*));
    key_labels[0] = "NONE";
    for (int i = 0; i < key_labels_count; i++) {
        key_labels[i+1] = (char*)key_label_mapping[i].name;
    }
    key_labels[key_labels_count+1] = NULL;
}

// Variables para recordar qué estamos mapeando al abrir el submenú
static int active_mapping_id = -1;
static MenuItem* active_keyboard_item = NULL;

// Callback: Cuando el usuario elige una tecla en el submenú
static int OptionKeyboard_keyChosen(MenuList* list, int i) {
    ButtonMapping* button = &config.controls[active_mapping_id];
    
    // Asignamos el valor interno de libretro (ej: RETROK_SPACE)
    button->retro_key = key_label_mapping[i].key;
    
    // Actualizamos la etiqueta visual del menú principal (+1 por el NONE)
    if (active_keyboard_item) {
        active_keyboard_item->value = i + 1; 
    }
    
    // Salimos del submenú para volver al menú de Keyboard Map
    return MENU_CALLBACK_EXIT; 
}

// El submenú que muestra todas las teclas ("space", "return"...)
static MenuList OptionKeyboard_keyMenu = {
    .type = MENU_LIST,
    .desc = "Select a key to assign.",
    .on_confirm = OptionKeyboard_keyChosen,
    .items = NULL
};

// Callback: Al pulsar 'A' en el menú principal, abrimos el submenú de teclas
static int OptionKeyboard_bind(MenuList* list, int i) {
    active_keyboard_item = &list->items[i];
    active_mapping_id = active_keyboard_item->id;
    
    // Generamos la lista de teclas la primera vez
    if (OptionKeyboard_keyMenu.items == NULL) {
        OptionKeyboard_keyMenu.items = calloc(key_labels_count + 1, sizeof(MenuItem));
        for (int k = 0; k < key_labels_count; k++) {
            OptionKeyboard_keyMenu.items[k].name = (char*)key_label_mapping[k].name;
        }
    }
    
    // Abrimos el selector de teclas
    Menu_options(&OptionKeyboard_keyMenu);
    return MENU_CALLBACK_NOP;
}

// Callback: Al pulsar 'X', limpiamos la tecla asignada
static int OptionKeyboard_unbind(MenuList* list, int i) {
    MenuItem* item = &list->items[i];
    ButtonMapping* button = &config.controls[item->id];
    
    button->retro_key = 0;
    item->value = 0; // 0 equivale a "NONE"
    
    return MENU_CALLBACK_NOP;
}

// El menú principal de mapeo
static MenuList OptionKeyboard_menu = {
    .type = MENU_INPUT,
    .desc = "Press A to pick a key and X to clear.",
    .on_confirm = OptionKeyboard_bind,
    .on_change = OptionKeyboard_unbind,
    .items = NULL
};

static int OptionKeyboard_openMenu(MenuList* list, int i) {
    LOG_info("OptionKeyboard_openMenu\n");
    init_key_labels();

    if (OptionKeyboard_menu.items == NULL) {
        OptionKeyboard_menu.items = calloc(RETRO_BUTTON_COUNT + 1, sizeof(MenuItem));
        int k = 0;
        for (int j = 0; config.controls[j].name; j++) {
            ButtonMapping* button = &config.controls[j];
            ...
            item->name = button->name;
            item->desc = NULL;
            item->values = key_labels; // ¡El array de strings de la derecha!
            
            // Si ya hay una tecla asignada, buscamos su índice para mostrarla
            item->value = 0; // Por defecto a NONE
            if (button->retro_key > 0) {
                for (int v = 0; v < key_labels_count; v++) {
                    if (key_label_mapping[v].key == button->retro_key) {
                        item->value = v + 1;
                        break;
                    }
                }
        ...
        // Si el menú ya estaba creado, solo actualizamos los valores 
        int k = 0;
        for (int j = 0; config.controls[j].name; j++) {
            ButtonMapping* button = &config.controls[j];
    ...
    Menu_options(&OptionKeyboard_menu);
    return MENU_CALLBACK_NOP;
}
// --- FIN DE IMPLEMENTACIÓN DEL MENÚ DE TECLADO ---

static MenuList options_menu = {
    .type = MENU_LIST,
    .items = (MenuItem[]) {
        {"Frontend", "MiaUI (" BUILD_DATE " " BUILD_HASH ")",.on_confirm=OptionFrontend_openMenu},
        ...
        {"Keyboard",.on_confirm=OptionKeyboard_openMenu}, 
...
static void Menu_saveState(void) {
    ...
    if (!bitmap) {
        temp_surface = SDL_CreateRGBSurfaceFrom(renderer.src, renderer.true_w, renderer.true_h, FIXED_DEPTH, renderer.src_p, RGBA_MASK_565);
        bitmap = temp_surface;
    }
    ...
    if (bitmap != menu.bitmap && bitmap != NULL) {
        ...
        if (dirty) {
            ...
            if (selected==ITEM_SAVE || selected==ITEM_LOAD) {
                ...
                if (menu.preview_exists) { // has save, has preview
                    ...
                    //LOG_info("raw_preview %ix%i fixed to RGB565\n", raw_preview->w, raw_preview->h);
...
static void trackFPS(void) {
    ...
    if (now - sec_start>=1000) {
        ...
        fps_ticks = 0;
         ...
         //LOG_info("fps: %f cpu: %f core: %f\n", fps_double, cpu_double, core.fps);
    }
}
...
bool exists_core_overlay(char* fullpath) {
    const char *pak_path = getenv("PAK_PATH");
    if (pak_path == NULL) {
        LOG_error("La variable PAK_PATH no está definida > IGN overlay.\n");
        return NULL;
    }
    ...
    snprintf(fullpath, MAX_PATH, "%s/overlay.png", pak_path);

	LOG_info("exists_core_overlay >>> check: %s\n", fullpath);
...
int main(int argc , char* argv[]) {
    ...
    // Overrides_init();
    if (exists_core_overlay(ovlpath)) {
        GFX_initGameOverlay((const char*) ovlpath);
    }
